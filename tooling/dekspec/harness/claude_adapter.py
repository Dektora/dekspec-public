"""The Claude baseline adapter for the harness seam (ds-mnxj / IC-017).

This adapter is a *pass-through binding*, not a reimplementation of dispatch
semantics. Real Claude sub-agent dispatch happens via the harness Agent tool
(the model reading skill instructions), not a Python API — so the default
in-process Python path is intentionally inert. The adapter's job is to guarantee
result *shape*, *ordering*, and *failure isolation*; the actual per-task work is
delegated to an injectable ``executor`` callable (tests inject a fake; the live
skill layer supplies the real binding).
"""

from __future__ import annotations

# NOTE: ``TaskResult`` / ``HarnessUnsupported`` live in the package ``__init__``,
# which imports this module to re-export ``ClaudeAdapter``. To avoid a circular
...
def _default_executor(task: dict, *, isolation: str, subagent_type: str) -> object:
    ...
    raise HarnessUnsupported(
        "dispatch_subagents",
        "claude",
        "no in-process executor available; dispatch is driven by the Claude "
        "harness tool layer, not callable from pure Python",
    )


class ClaudeAdapter:
    """Pass-through binding of the seam to the Claude harness."""
    ...
    def invoke_skill(self, name: str, args: str) -> object:
        ...
        raise HarnessUnsupported("invoke_skill", "claude", "unknown skill")
