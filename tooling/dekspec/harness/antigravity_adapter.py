"""The Antigravity host adapter for the harness seam (ds-1b0n / IC-017).

Like ``ClaudeAdapter`` and ``CodexAdapter``, this is a *pass-through binding*,
not a reimplementation of dispatch semantics. Real Antigravity sub-agent
dispatch happens via the Antigravity harness tool layer, not a Python API — so
the default in-process Python path is intentionally inert. The adapter
guarantees result *shape*, *ordering*, and *failure isolation*; per-task work is
delegated to an injectable ``executor`` callable (tests inject a fake; the live
harness layer supplies the real binding).

The Antigravity host reports ``native`` for all four primitives (spike
2026-06-15: dynamic parallel subagents + extension hooks).
"""

from __future__ import annotations

# NOTE: ``TaskResult`` / ``HarnessUnsupported`` live in the package ``__init__``,
# which imports this module to re-export ``AntigravityAdapter``. To avoid a
# circular import at load time, those names are imported lazily inside the
# methods below.
...
def _default_executor(task: dict, *, isolation: str, subagent_type: str) -> object:
    ...
    raise HarnessUnsupported(
        "dispatch_subagents",
        "antigravity",
        "no in-process executor available; dispatch is driven by the Antigravity "
        "harness tool layer, not callable from pure Python",
    )


class AntigravityAdapter:
    """Pass-through binding of the seam to the Antigravity harness."""
    ...
    def dispatch_subagents(
        ...
        *,
        ...
        # valid (sequential) realization that satisfies index-alignment. Even if a
        # concurrent backend is added later, results MUST be re-ordered to input
        # index so result[i] <-> tasks[i].
        results: list = []
        for task in tasks:
            try:
    ...
    def invoke_skill(self, name: str, args: str) -> object:
        ...
        raise HarnessUnsupported("invoke_skill", "antigravity", "unknown skill")
