"""The Codex host adapter for the harness seam (ds-afv3 / IC-017).

Like ``ClaudeAdapter``, this is a *pass-through binding*, not a reimplementation
of dispatch semantics. Real Codex sub-agent dispatch happens via the Codex
harness tool layer, not a Python API — so the default in-process Python path is
intentionally inert. The adapter guarantees result *shape*, *ordering*, and
*failure isolation*; per-task work is delegated to an injectable ``executor``
callable (tests inject a fake; the live harness layer supplies the real binding).

The Codex host reports ``native`` for all four primitives (spike 2026-06-15).
"""

from __future__ import annotations

# NOTE: ``TaskResult`` / ``HarnessUnsupported`` live in the package ``__init__``,
# which imports this module to re-export ``CodexAdapter``. To avoid a circular
...
def _default_executor(task: dict, *, isolation: str, subagent_type: str) -> object:
    ...
    raise HarnessUnsupported(
        "dispatch_subagents",
        "codex",
        "no in-process executor available; dispatch is driven by the Codex "
        "harness tool layer, not callable from pure Python",
    )


class CodexAdapter:
    """Pass-through binding of the seam to the Codex harness."""
    ...
    def invoke_skill(self, name: str, args: str) -> object:
        ...
        raise HarnessUnsupported("invoke_skill", "codex", "unknown skill")
