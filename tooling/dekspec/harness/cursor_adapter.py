"""The Cursor host adapter for the harness seam (ds-h3xk / IC-017).

Like ``ClaudeAdapter``, ``CodexAdapter``, and ``AntigravityAdapter``, this is a
*pass-through binding*, not a reimplementation of dispatch semantics. Real Cursor
sub-agent dispatch happens via the Cursor harness tool layer, not a Python API —
...
The Cursor host reports ``native`` for all four primitives — including
``dispatch_subagents``. The 2026-06-15 spike overturned the earlier
"Cursor sequential-only" premise: Cursor 2.5+ has async nested parallel
sub-agents, so dispatch is NATIVE, not degraded.
"""

from __future__ import annotations

# NOTE: ``TaskResult`` / ``HarnessUnsupported`` live in the package ``__init__``,
# which imports this module to re-export ``CursorAdapter``. To avoid a circular
...
def _default_executor(task: dict, *, isolation: str, subagent_type: str) -> object:
    ...
    raise HarnessUnsupported(
        "dispatch_subagents",
        "cursor",
        "no in-process executor available; dispatch is driven by the Cursor "
        "harness tool layer, not callable from pure Python",
    )


class CursorAdapter:
    """Pass-through binding of the seam to the Cursor harness."""
    ...
    def invoke_skill(self, name: str, args: str) -> object:
        ...
        raise HarnessUnsupported("invoke_skill", "cursor", "unknown skill")
