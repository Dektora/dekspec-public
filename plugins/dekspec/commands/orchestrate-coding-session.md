...
1. Invoke the `orchestrate-coding-session` skill via the Skill tool,
...
Renamed from `/run-coding-session` → `/orchestrate-coding-session` in
...
> Added by **INT-123** (LOCKED 2026-05-30). The orchestrate-coding-session command emits IB lifecycle status transitions after each bead's test run; failure routes through the INT-121 Python action-handler registry.
...
| Any bead red (CI or pytest fail) | IMPLEMENTING → **TESTFAIL** | `dekspec.action_handlers.dispatch("TESTFAIL", context)` → INT-114 handler (`/orchestrate-coding-session --retry` + re-eval) |
...
After the TESTFAIL handler re-runs the session via `/orchestrate-coding-session --retry`:
