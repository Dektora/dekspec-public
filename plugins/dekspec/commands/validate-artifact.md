---
description: Single-artifact schema validation via `dekspec check validate`. **Use when:** you've just edited one artifact and want a fast schema check. **Narrower than:** `/dekspec:doctor` (which covers graph linkage + per-artifact schema + drift across the whole tree).
allowed-tools: Bash(dekspec check validate:*), Bash(dekspec check:*)
...
2. Run `dekspec check validate $ARGUMENTS` via Bash.
