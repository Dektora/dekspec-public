...
2. If the incubation folder `dekspec/provisional/<slug>/` does not exist OR does not yet contain a `INT-provisional-*.md` file for this work, scaffold via:
   ```
...
3. Read the scaffolded file at `dekspec/provisional/<slug>/INT-provisional-<title-slug>.md` (the CLI prints the path).
