...
modes:
  ...
  - { flag: "--decompose", args: "<Intent-path>", description: "Branch by IB-need (Decision #12) per IU recorded in the Layer impact analysis. For type: bug, scaffolds the failing-test bead as IB-1 via /write-code-beads --bug-reproduction and resolves the <reproduction-test-path-from-IB-1> placeholder in the Verification block. Multi-WS IUs route through /write-ibs first; single-WS IUs go directly to /write-code-beads with the Intent path. Re-checks size caps post- decomposition; reverts to OVERSIZED if exceeded. Promotes ACCEPTED → IMPLEMENTING." }
  ...
  - { flag: "--supersede", args: "<Intent-path> --by <INT-NNN|MSN-NNN>", description: "Transition a non-LOCKED pre-implementation Intent (DRAFT/OVERSIZED/PROPOSED/ACCEPTED) absorbed by a named successor artifact to SUPERSEDED; records Superseded-By and moves the index row to Archive (ADR-035). Refuses LOCKED (the ADR-028 successor-Intent path), IMPLEMENTING/TESTPASS/MERGED (work in flight or shipped), and already-terminal." }
