---
description: Run `dekspec check compile` — parse LOCKED DekSpec artifacts and emit the typed IR JSON for downstream audit/enforcement.
allowed-tools: Bash(dekspec check compile:*), Bash(dekspec check:*)
...
1. Run `dekspec check compile $ARGUMENTS` via Bash.
