...
2. Relay the skill's output to the operator. The skill sequences the existing authoring skills (`/write-intent --analyze/--accept/--decompose`, `/write-ws`, `/write-ic`, `/write-ibs`) and stops at the coding boundary — it never dispatches `/exec-coding-session`. `--accept` stays engineer-gated.
