...
DekSpec is shipped as a Python library + CLI + Claude Code skills + markdown templates, vendored into consumer repos via a single install script. The current version is **v0.121.1**.
...
```bash
pipx install "git+https://github.com/Dektora/dekspec-public.git@v0.121.1"
```

CLI only into a project venv:
```bash
pip install "git+https://github.com/Dektora/dekspec-public.git@v0.121.1"
```
